...
global scale, buff_Order

; status variables
global hour_gather_time, hour_convert_time, hour_other_time, hour_gather_percent, hour_convert_percent, hour_other_percent, gather_percent, convert_percent, other_percent
global gather_time, other_time, convert_time
...
; added custom typing to format numbers into ARGB Hex + arrays
...
defaultConfig.Push("BuffOrder", "You can change the order of buffs here`n"
"To disable a buff simply put a " "; in it anywhere, just like these comments!", Array(
	"buff_Order", "Array", Array(
		"boost", "",
		"haste", "",
		"focus", "",
		"bombcombo", "",
...
stat_regions := Map(
	  "lasthour", [regions["rightTabRegion"][1]+100,regions["rightTabRegion"][2]+100,regions["rightTabRegion"][3]-200,1206]
...
graph_regions := Map(
	  "honey", [regions["honey"][1]+320,regions["honey"][2]+130,3600,800]
...
boost_args := Array(
	"whiteboost", 0xffffffff,
	"redboost", 0xffe46156,
	"blueboost", 0xff56a4e4
)

buff_Data := Map(
	  "boost",				[280, boost_args, 10]
	, "haste",				[280, 0xfff0f0f0, 10]
	, "focus",				[280, 0xff22ff06, 10]
	, "bombcombo",			[280, 0xffa0a0a0, 10]
	, "balloonaura",		[280, 0xff3350c3, 10]
	, "inspire",			[280, 0xfff4ef14, (buff) => Max(ceil(maxX(buff_values[buff])/5)*5, 5)]
	, "reindeerfetch",		[280, 0xffcc2c2c, 10]
	, "honeymark",			[120, 0xffffd119,,true]
	, "pollenmark",			[120, 0xffffe994,,true]
	, "festivemark",		[120, 0xffc84335]
	, "popstar",			[110, 0xff0096ff]
	, "melody",				[110, 0xfff0f0f0]
	, "bear",				[110, 0xffb26f3e]
	, "babylove",			[110, 0xff8de4f3]
	, "jbshare",			[110, 0xfff9ccff]
	, "guiding",			[110, 0xffffef8e]
	, "snowflake",			[110, 0xffc6fbff]
...
buffY := regions["buffsRegion"][2]+135
...
for buff in buff_order {
	...
	if h == 280 || h == 400 {
		Gdip_DrawLine(G, pPen, buffXPos-60, y+h//2, buffXPos+buffXWidth+60, y+h//2)
	} else if h == 480 {
		Gdip_DrawLine(G, pPen, buffXPos-60, y+h*A_Index//4, buffXPos+buffXWidth+60, y+h*A_Index//4)
	}
}
...
for buff in buff_order {
	...
	Gdip_FillRectangle(G, pBrush, buffXPos-60, y, buffXWidth+120, h)
	Gdip_DrawImage(G, bitmaps["pBM" buff], regions["buffsRegion"][1]+75, y+h//2-55, 110, 110)
	Gdip_DisposeImage(bitmaps["pBM" buff])
	Gdip_DrawLine(G, pPen, buffXPos-60, y+h+10, buffXPos+buffXWidth+60, y+h+10)
}
...
DetectHoney() {
	...
	Loop 25 {
		...
		Loop 2 {
			...
			try {
				detected[v := ((StrLen((n := RegExReplace(MultiStrReplace(OCR(pIRandomAccessStream).Text, Map("o", "0", "i", "1", "l", "1", "a", "4")), "\D"))) > 0) ? n : 0)] := detected.Has(v) ? [detected[v][1]+1, detected[v][2] " " i . A_Index] : [1, i . A_Index]
			}
		}
	}
...
/**********************************************************************
* @description: Identifies misread values and adds missing honey values
* @returns: ()
* @author SP (more or less)
**********************************************************************/
PopulateHoneyValues() {
	global
	if (honey_values.Count > 0) {
		...
		min_value := minX(honey_values)
		max_value := Max(maxX(honey_values), min_value+1000)
...
/*************************************************
* @description: Updates the status list variables.
* @returns: ()
* @author SP (more or less)
*************************************************/
UpdateStatusList() {
	...
	for idx, status in status_list {
		hour_%status%_time := 0
		if not IsSet(%status%_time) {
			%status%_time := 0
		}
	}
	enum := status_changes.__Enum()
	enum.Call(&m)
	for idx, status in status_changes {
		if (enum.Call(&m) = 0) {
			m := 3600
		}
		status := (status = 1) ? "Gather"
			: (status = 2) ? "Convert"
			: "Other"
		hour_%status%_time += m-idx
	}

	for idx, status in status_list {
		%status%_time += hour_%status%_time
	}
	...
	for idx, status in status_list {
		cumul_hour += hour_%status%_time*100/3600
		hour_%status%_percent := Round(cumul_hour) - cumul_hour_rounded . "%"
		cumul_hour_rounded := Round(cumul_hour)

		cumul_total += %status%_time*100/session_time
		%status%_percent := Round(cumul_total) - cumul_total_rounded . "%"
		cumul_total_rounded := Round(cumul_total)
	}
}

/*******************************************************
* @description: Checks an array if it has the given item
* @returns: (boolean)
* @author ManlyTorch
*******************************************************/
...
/*************************************************************
* @description: Returns true or false if it's a color property
* @returns: (boolean)
* @author ManlyTorch
*************************************************************/
isColor(propType) {
...
/****************************************************************
* @description: Creates an ini string for the given config value.
* @returns: (string)
* @author ManlyTorch & SP?
****************************************************************/
nm_getiniString(name, type, value, defaultVal, ArrayName:="", ArrayIdx:=0) {
	global
	if isColor(type) {
		if type == "ParentColor" && value == ""{
			ini := name "="
		} else if IsInteger(value) {
			ini := name "=" StrLower(Format("0x{:08X}", value))
		} else if not pngExists(value) { ; Invalid value set, reset to default in config
			if A_Args.Length > 1 { ; generating template, ask to replace with default color (just incase they miss named it or smth idk man, thought it'd be nice)
				result := MsgBox("Property " name " has been set to an image that does not exist. Would you like to automatically replace it with the default value?", "Invalid value in config", "YesNo")

				if ArrayName {
					orgVal := name
					%ArrayName%[ArrayIdx] := name
				} else {
					orgVal := value
					%name% := defaultVal
				}
				...
				if result == "No" {
					ini := name "=" orgVal
				} else if not IsInteger(defaultVal) {
					ini := name "=" defaultVal
				} else {
					ini := name "=" StrLower(Format("0x{:08X}", defaultVal))
				}
		...
		for idx, val in value {
			str := nm_getiniString(val, "", val, val, name, idx)
			if InStr(val, ";") {
				ini .= "	" "; " MultiStrReplace(str, Map(";", "", " ", "", "	", ""))
			} else {
				validArray.Push(val)
				ini .= str
			}
		...
		%name% := validArray
		ini := SubStr(ini, 1, StrLen(ini) - 1) "`n}"
	} else if ArrayName {
		ini := "	" name
	} else {
		ini := name "=" value
...
/**************************************************************************************
* @description: Generates an ini string for the config and writes it to the given path.
* @returns: ()
* @author ManlyTorch & SP?
**************************************************************************************/
nm_writeConfig(inipath) {
	global
	local ini := "[ini]"
	for idx, configArray in defaultConfig { ; overwrite any existing .ini with updated one with all new keys and old values
		curMod := Mod(idx, 3)
		if curMod != 0 {
			if curMod == 2 {
				lines := StrSplit(configArray, "`n")
				for line in lines {
					ini .= "; " line "`r`n"
				}
			}
		...
		propName := ""
		propType := ""
		for idx, propVal in configArray {
			if Mod(idx, 3) == 1 {
				propName := propVal
				continue
			} else if Mod(idx, 3) == 2 {
			...
			ini .= nm_getiniString(propName, propType, %propName%, propVal)
		}
		ini .= "`r`n"
	}
...
/*************************************************************************************************
* @description: Loads the default config to global variables & converts the array config to a map.
* @returns: (Map)
* @author ManlyTorch & SP?
*************************************************************************************************/
nm_convertDefaultConfig() {
	global
	MapDefaultConfig := Map("PropTypes", Map())
	sectionName := ""
	for idx, configArray in defaultConfig { ; load the default values as globals, will be overwritten if a new value exists when reading
		modResult := Mod(idx, 3)
		if modResult != 0 {
			if modResult == 1 {
				sectionName := configArray
			}
			continue
		}
...
/****************************************** *****
* @description: Imports the stat monitors config.
* @returns: ()
* @author ManlyTorch & SP?
********************************************* **/
nm_importConfig() {
	...
	defaultMap := nm_convertDefaultConfig()
...
/******************************************************************************
* @description: Reads the ini file and sets the values to their global variable
* @returns: ()
* @author ManlyTorch & SP?
******************************************************************************/
nm_ReadIni(path) {
	...
	Loop Parse str, "`n", "`r" A_Space A_Tab {
		line := A_LoopField
		varSetPos := InStr(line, "=")
		if InStr(line, "{") {
			isArray := true
			curArray := Array()
			curArrayName := SubStr(line, 1, varSetPos-1)
			continue
		} else if isArray {
			if InStr(line, "}") {
				isArray := false
			...
			try {
				variable := SubStr(line, 1, varSetPos-1)
				value := SubStr(line, varSetPos+1)
				%variable% := IsInteger(value) ? Integer(value) : value
			}
		}
	}
}
...
/*********************************************************************************
* @description: tries multiple picture extensions to see if the given path exists.
* @returns: (boolean)
* @author ManlyTorch
*********************************************************************************/
tryExt(path) {
...
/**********************************************************
* @description: Checks if the given path contains an image.
* @returns: (boolean)
* @author ManlyTorch
**********************************************************/
pngExists(path) {
...
/**************************************************************
* @description: Attempts to load an image from the given config
* @returns: (boolean)
* @author ManlyTorch
**************************************************************/
...
/***************************************************************************
* @description: Checks if the user disabled the given runtime generated stat
* @returns: (boolean)
* @author ManlyTorch
***************************************************************************/
renderDisabled(stat) {
...
/*********************************************************************
* @description: Sends the hourly report early, doesn't wipe the stats.
* @returns: ()
* @author ManlyTorch
*********************************************************************/
forceReport(*) {
...
/********************************************************************************
* @description: Draws the graph of the given buff, with its given buff multiplier
* @returns: ()
* @author SP (more or less)
********************************************************************************/
DrawBuffMultiGraph(G_Graph, buff, buffArgs, color, max_buff, textOffset:=0) {
	y := buffArgs[1][1]
	h := buffArgs[1][2]
	...
	Gdip_TextToGraphics(G, "x" . (count ? Round(total/count, 3) : "0.000"), "s32 Center Bold c" pBrush " x" buffXPos-190 " y" y + 36 + textOffset, "Segoe UI")
	Gdip_DeleteBrush(pBrush)
	...
	if (points.Length > 2) {
		pBrush := Gdip_CreateLinearGrBrushFromRect(4, 4, buffXWidth, h, 0x00000000, color - 0x40000000)
...
/********************************************************************
* @description: Draws a flat color for the duration of the given buff
* @returns: ()
* @author SP (more or less)
********************************************************************/
DrawBuffGraph(G_Graph, buff, color, buffArgs) {
	h := buffArgs[1][2]
...
/****************************************
* @description: Draws the graph for marks
* @returns: ()
* @author SP (more or less)
****************************************/
DrawMarkGraph(G_Graph, buff, color, buffArgs) {
	h := buffArgs[1][2]
	pBrush := Gdip_BrushCreateSolid(color-0x60000000)
	for time, value in buff_values[buff]
		(value && value < 4 && value > 0) && Gdip_FillRectangle(G_Graph, pBrush, 4+buffXWidth*time//600, 4+h*(3-value)//3, 6, h*value//3)
	Gdip_DeleteBrush(pBrush)
}

/*************************************************
* @description: Draws the graphs, not much to say.
* @returns: ()
* @author SP (more or less)
*************************************************/
DrawGraphs() {
	; create honey gradients
	honey_gradients := Map()
	for k,v in honey_values {
		if (k < 60) {
	...
	honey_gradients[0] := honey_gradients[1]
	honey_gradients[61] := honey_gradients[60]

	; set variable graph bounds
	min_gradient := 0
	max_gradient := Max(maxX(honey_gradients), min_gradient+1000)
	range_gradient := Floor(max_gradient - min_gradient)
	min_12h := minX(honey_12h)
	max_12h := Max(maxX(honey_12h), min_12h+1000)
	range_12h := max_12h - min_12h

	; draw non buff graphs
	for k,v in graph_regions {
		if renderDisabled(k) {
			continue
		}
	...
	for buff in buff_order {
		...
		max_buff := 0
		if args.Has(3) {
			max_buff := args[3] is Func ? args[3](buff) : args[3]
		}

		pBMGraph := Gdip_CreateBitmap(buffXWidth+8, h+8)
		G_Graph := Gdip_GraphicsFromImage(pBMGraph)
		Gdip_SetSmoothingMode(G_Graph, 4)
		Gdip_SetInterpolationMode(G_Graph, 7)
		...
		if max_buff {
			if args[2] is Array {
				for idx, val in args[2] {
					...
					DrawBuffMultiGraph(G_Graph, buff, args, color, max_buff, (trueIdx-1) * -36)
				}
			} else {
				DrawBuffMultiGraph(G_Graph, buff, args, color, max_buff)
			}
		} else if args.Has(4) {
			DrawMarkGraph(G_Graph, buff, color, args)
		} else {
			DrawBuffGraph(G_Graph, buff, color, args)
		}
		Gdip_DeleteGraphics(G_Graph)
		Gdip_DrawImage(G, pBMGraph, buffXPos-4, y-4)
		Gdip_DisposeImage(pBMGraph)
	}
}

/**************************************************************
* @description: Draws the right tab buffs on the hourly report.
* @returns: ()
* @author SP (more or less)
**************************************************************/
DrawBuffsSection() {
	if renderDisabled("buffs") {
		return
	}
...
/************************************************************
* @description: Draws the pie graph & text for session stats.
* @returns: ()
* @author SP (more or less)
************************************************************/
DrawSessionStats() {
	global
	if renderDisabled("session") {
		return
	}
	...
	angle := -90
	for idx, status in status_list {
		color := (status = "Gather") ? 0xffa6ff7c
				: (status = "Convert") ? 0xfffeca40
				: 0xff859aad
		pBrush := Gdip_BrushCreateSolid(color)
		Gdip_FillPie(G, pBrush, stat_regions["session"][1]+stat_regions["session"][3]//2-464, stat_regions["session"][2]+402, 280, 280, angle, %status%_time/session_time*360)
		angle += %status%_time/session_time*360
		...
		Gdip_TextToGraphics(G, status, "s48 Right Bold ccfffffff x" stat_regions["session"][1]+stat_regions["session"][3]//2+56 " y" stat_regions["session"][2]+419+(A_Index-1)*88, "Segoe UI")
		Gdip_TextToGraphics(G, DurationFromSeconds(%status%_time), "s48 Left Bold cefffffff x" stat_regions["session"][1]+stat_regions["session"][3]//2+135 " y" stat_regions["session"][2]+419+(A_Index-1)*88, "Segoe UI")
		Gdip_TextToGraphics(G, %status%_percent, "s48 Right Bold cefffffff x" stat_regions["session"][1]+stat_regions["session"][3]//2+476 " y" stat_regions["session"][2]+419+(A_Index-1)*88, "Segoe UI")
	}
}
