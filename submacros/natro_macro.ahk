...
(GuiCtrl := MainGui.Add("CheckBox", "xp yp+14 vHeatTreatedPlanterCheck Disabled Checked" HeatTreatedPlanterCheck hidden, "HeatTreated")).Section := "Planters", GuiCtrl.OnEvent("Click", nm_saveConfig)
	...
	if (StickerStackCheck && (nowUnix()-LastStickerStack)>StickerStackTimer) {
		loop 2 {
			...
			If (searchRet[1] = 0) {
				...
				if stackTime {
					nm_PostBuff("StickerStack", Floor(stackTime / 6))
				}
				break
			}
		}
...
nm_toBooster(location){
	...
	static blueBoosterFields:=["Pine Tree", "Bamboo", "Blue Flower", "Stump"]
...
nm_fieldBoostCheck(fieldName, variant:=0){
	...
	loop Floor(windowWidth/38) { ; flooring because you won't have half of an icon 
		ico:=(A_Index-1)*38
		if (Gdip_ImageSearch(pBMScreen, bitmaps["boost"][StrReplace(fieldName, " ") variant],,ico,,ico+38,,(variant=1 || variant=0) ? 35 : 50)) { ; testing tighter variation ; check with original 30 not 35
...
nm_locateVB(){ 
	...
	static VBData := [ { field: "Pepper", enabled: StingerPepperCheck
		, bees: 35
		, reps: 1
		, lrdist: 20
		, fbdist: 7
		, initRight: 10
		, initFwd: 7 }, { field: "MountainTop", enabled: StingerMountainTopCheck
		, bees: 25
		, reps: 2
		, lrdist: 17
		, fbdist: 5.5
		, initRight: 8.5
		, initFwd: 10.5 }, { field: "Rose", enabled: StingerRoseCheck
		, bees: 15
		, reps: 2
		, lrdist: 13
		, fbdist: 6
		, initRight: 6.5
		, initFwd: 12 }, { field: "Cactus", enabled: StingerCactusCheck
		, bees: 15
		, reps: 1
		, lrdist: 26
		, fbdist: 5.5
		, initRight: 13
		, initFwd: 3.7 }, { field: "Spider", enabled: StingerSpiderCheck
		, bees: 5
		, reps: 2
		, lrdist: 21
		, fbdist: 5.7
		, initRight: 10.5
		, initFwd: 9.5 }, { field: "Clover", enabled: StingerCloverCheck
		, bees: 0
		, reps: 2
		, lrdist: 22
		, fbdist: 5.7
